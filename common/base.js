    ...
    var PageBase = Class.extend({
        ...
        onInit: function ($container, tpl, data, indexRef) {
            ...
            that.$container.on('click', 'button[data-open]', function () {
                ...
                // 界面值带回给对话框
                data.id = that.onBackElement.attr('data-value');
                data.name = that.onBackElement.val();
                util.openPage(module, data);
            });
            // 清空按钮
            that.$container.on('click', 'button[data-action="clear"]', function () {
                var $el = $(this);
                var $input =$el.parents('.input-group').find('input:visible');
                $input.val('');
        ...
        onBack: function (data) {
            // 设置返回值公共方法
            if (data && (typeof data.name !== 'undefined') && (typeof data.id !== 'undefined') && this.onBackElement) {
                this.onBackElement.val(data.name);
                this.onBackElement.attr('data-value', data.id);
                this.onBackElement.attr('title', data.name);
         ...
         * @param filed
         */
        addValidateField: function($form, filed){
            $form.data('formValidation').addField(filed);
        },
         ...
         * @param filed
         */
        removeValidateField: function($form, filed){
            $form.data('formValidation').removeField(filed);
        },
         ...
         * @param resetValue
         */
        resetValidateField: function($form, filed, resetValue){
            $form.data('formValidation').resetField(filed, resetValue);
        },
        ...
        buildSearchParams: function ($els) {
            ...
            $els.each(function () {
                $el = $(this);
                params[$el.attr('data-search')] =$el.attr('data-value') ? $el.attr('data-value') : $el.val();
            });
         ...
         * 设置表格筛选
         */
        makeSorting : function (tableId,callBack) {
            this.$container.on('click', tableId + ' .sorts', function () {
                var $el=$(this);
                if($el.hasClass('sorting_desc')){
                    $el.parents('table').find('.sorts').addClass('sorting').removeClass('sorting_desc').removeClass('sorting_asc');
                    $el.removeClass('sorting').removeClass('sorting_desc').addClass('sorting_asc');
                }else {
                    $el.parents('table').find('.sorts').addClass('sorting').removeClass('sorting_desc').removeClass('sorting_asc');
                    $el.removeClass('sorting').removeClass('sorting_asc').addClass('sorting_desc');
                }
                if (callBack) {
                    callBack();
                }

define(function(require, exports, module) {
    ...
    var Module = PageBase.extend(new function() {
        ...
        function searchBookList(){
            ...
            util.sendRequest({
                ...
                success: function (resp) {
                    ...
                    if (resp.code === 0) {
                        // 从得到数据提取id
                        var addIds=[],bookNames=[],bookPrintIds=[];
                        for(var i=0;i<resp.data.length;i++){
                            for(var j in resp.data[i]){
                                ...
                                if(j==='bookPrintId'){
                                    bookPrintIds.push(resp.data[i][j]);
                                }
                            }
                        }

                        if(info.book && info.book.idBook){
                            var idArr=[];
                            idArr=info.book.idBook;
                            $.each(idArr,function (i) {
                                var index=$.inArray(idArr[i],addIds);
                                if(index >= 0){
                                    addIds.splice(index, 1);
                                    bookNames.splice(index, 1);
                                    bookPrintIds.splice(index, 1);
                                }
                            });
                        }
                        ...
                        for(var i=0;i<addIds.length;i++){
                            var bookTr=$('<tr data-printId="' + bookPrintIds[i] + ' "data-id="' + addIds[i] +'" data-name="' +bookNames[i] +'"><td>'+ (i + 1) +'</td><td data-check="true"><i class="fa fa-square-o" data-action="check"></i></td><td>'+ bookNames[i]+'</td></tr>');
                            $tbody.append(bookTr);
                        }
                    } else {
                        console.log(resp);
        ...
        function saveReportInfo(){
            var $row;
            var printBOOKIds = [],printNames=[],printIds = [];
            that.$('#selectBookTbody tr').each(function(){
                $row = $(this);
                if ($row.find('i[data-action="check"]').hasClass('fa-check-square-o')) {
                    printBOOKIds.push($row.attr('data-id'));
                    printNames.push($row.attr('data-name'));
                    printIds.push($row.attr('data-printId'));
                }
            });
            if(printIds.length>0){
                bookData={
                    bookIds:printBOOKIds,
                    bookNames:printNames,
                    bookPrint:printIds,
                    book:true
                };
            }

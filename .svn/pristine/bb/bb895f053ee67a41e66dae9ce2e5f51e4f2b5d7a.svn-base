			...
			<table class="table table-bordered table-nowrap">
				<thead>
					<tr>
						...
						<th>下一流程</th>
						<th>下一流程任务人<span class="must-td">*</span></th>
					</tr>
				</thead>
				<tbody>
					<% for(var i = 0; i < info.userTaskEntities.length; i++) { %>
                    ...
                    class="fa fa-user-o"></i>
						...
						<td>
							<select class="form-control" name="nextFlow">
								...
								<% if( info.declareDeptId === info.userTaskEntities[i].sonUserTaskList[j].deptIds)  {%>	
								<option value = "<%- info.userTaskEntities[i].sonUserTaskList[j].deptIds %>" selected><%- info.userTaskEntities[i].flowName %>(<%- info.userTaskEntities[i].sonUserTaskList[j].deptAndRoles %>)</option>
								<% } else { %>

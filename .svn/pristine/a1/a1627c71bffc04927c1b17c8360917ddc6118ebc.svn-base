define(function (require, exports, module) {
    ...
    module.exports = ItemBase.extend({
        ...
        addTableRow: function () {
            ...
            var $child = util.template(listTpl, {list: [{orderNo: $tb.children().length + 1}], ratioMap: ratioMap, bookFascicleList: this.data.info.bookFascicleList});
            $tb.append($child);
            var that = this;
        ...
        initPageEvent: function () {
            ...
            // 在线校对
            this.$container.on("click", "button[name='onlineProof']", function () {
                var $el = $(this);
                window.proofreadErrataCallback = function (list) {
                    if (!list) {
                        list = [];
                    }
                    var $tb = that.$("tbody[data-name='trueOrFalseTb']");
                    var childCount = $tb.children().length;
                    for (var i = 0; i < list.length; i++) {
                        list[i].orderNo = childCount + i + 1;
                    }
                    $tb.append(util.template(listTpl, {list: list, ratioMap: ratioMap, bookFascicleList: that.data.info.bookFascicleList}));
                    $tb.children().last().find("input").each(function () {
                        that.addValidateField($(this));
                    });
                    that.recalculateDiff();
                };
                window.open('./editor/page-editor.html?page=editor-proofread&taskId=' + $el.attr("data-taskId") + '&bookId=' + $el.attr("data-bookId") +
                    '&bookFascicleId=' + $el.parent().find("select[name='onlineProofFascicule']").val(), '_blank');
            });
            // 在线校对分册选择
            this.$container.on("change", "select[name='onlineProofFascicule']", function () {
                if ($("select[name='onlineProofFascicule']").find("option:selected").attr("data-book-upload-type") === "2") {
                    $("button[name='onlineProof']").removeAttr("disabled");
                } else {
                    $("button[name='onlineProof']").attr("disabled", "disabled");
                }
            });
        },

define(function (require, exports, module) {
    ...
    var Module = PageBase.extend(new function () {
        ...
        function searchBookList() {
            ...
            util.showLoading()
            util.sendRequest({
                action: 'reprint/searchBookToReprintList.action',
                data: {},
                success: function (resp) {
                    ...
                    if (resp.code === 0) {
                        ...
                        for (var i = 0; i < addIds.length; i++) {
                            var bookTr = $('<tr data-id="' + addIds[i] + '" data-name="' + bookNames[i] + '" title="' + bookNames[i] + '"><td>' + (i + 1) + '</td><td data-check="true"><i class="fa fa-square-o" data-action="check"></i></td><td>' + bookNames[i] + '</td></tr>');
                            $tbody.append(bookTr);
                        }
                    } else {
                        console.log(resp);

define(function (require, exports, module) {
	...
	var FLOW_BIG_DEPUTY = 'bigBrailleChiefEditorTrialDeputy'; //副总编辑审核 
	...
	var FLOW_CITE_DEPUTY = 'importBrailleChiefEditorTrialDeputy'; //副总编辑审核 
    ...
    var Module = Base.extend(new function () {
        ...
        this.onCheckRowChanged = function() {
        	...
        	var bigRecord = true, citeRecord = true, bigTrial = true, citeTrial = true, bigDeputy = true, citeDeputy = true, bigChief = true, citeChief = true;
        	...
        	that.$('#workListTBody').find('tr[checked]').each(function(){
        		...
        		// 包含非重大副总编辑室审核
        		if (flowId !== FLOW_BIG_DEPUTY) {
        			bigDeputy = false;
        		}
        		// 包含非引用副总编辑室审核
        		if (flowId !== FLOW_CITE_DEPUTY) {
        			citeDeputy = false;
        		}
        		// 包含非重大总编辑室审核
        	...
        	if (count === 1) {
        		...
        		if ((bigTrial || citeTrial || bigDeputy || citeDeputy || bigChief || citeChief || bigpubli || bigRegister || citeRegister) && allMe && allUnfinished ) {
        			this.$('#btnTopicHandle').removeAttr('disabled');
        		}
        	}
        	// 选择一条或多条
        	if (count > 1) {
          		// 备案登记阶段可以删除
          		if (allMe && allUnfinished && (bigTrial || citeTrial || bigDeputy || citeDeputy || bigChief || citeChief || bigRegister || citeRegister)) {

...
<div class="form-horizontal" id="formEdit">
  			...
  			<div class="col-xs-10">
  				<table class="table table-striped">
                      ...
                      <button class="btn btn-default" type="button" data-open="public/file/file-upload" data-data="{}"><i class="fa fa-file-o"></i></button>
  							...
  							<td>
  								<input type="text" class="form-control" name="hTaskPageNum" value="<%- info.fascicleList[j].hTaskPageNum %>" data-fv-notempty="true" data-fv-notempty-message=" " maxlength="7" pattern="^[0-9]*$" data-fv-regexp-message="请输入大于0的整数" />
  							</td>
  ...
  <% if(info.userTaskEntities[i].flowId === 'cn_applyCIP') { %>
  ...
  <% } else if(info.userTaskEntities[i].flowId === 'cn_writePrint') { %>
  <div class="row" data-flow="<%- info.userTaskEntities[i].flowId %>">
    <div class="col-xs-6">
      <div class="form-group">
        <label class="col-xs-4 control-label">下一流程</label>
    ...
    <div class="col-xs-6">
      ...
      <div class="form-group">
        ...
        <div class="col-xs-8">
          <div class="input-group with-clear">
            <input type="text" class="form-control" name="taskUsers" title="<%- info.otherUserName %>" value="<%- info.otherUserName %>"
                   data-value="<%- info.handleContentTwo %>" placeholder="任务人" readonly
                   data-data="{searchRoleId: '', searchDepartmentId: ''}" data-fv-notempty="true" data-fv-notempty-message="请选择下一任务人">
            <span class="input-group-btn">
              <button class="btn btn-clear" type="button" data-action="clear" ><i class="fa fa-remove box"></i></button>
              <button class="btn btn-default" type="button" data-open="public/user/select-users" data-data='{"roleIds": "<%- info.userTaskEntities[i].roleIds %>", "deptIds": "<%- info.userTaskEntities[i].deptIds %>"}'>

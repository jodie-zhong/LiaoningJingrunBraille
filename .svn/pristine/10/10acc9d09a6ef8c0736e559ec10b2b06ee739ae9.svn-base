            ...
            <div>
              <table class="table table-bordered table-nowrap">
                ...
                <tr class="book-topic<%- (i) %>" data-id="<%- topic.topicId %>" data-taskId="<%- topic.taskId %>"
                  ...
                  <% } %>
                  <td data-bookId="<%- topicType.bookId %>" title="<%- topicType.bookTypeName %>"><%- topicType.bookTypeName %></td>
                  ...
                  <td>
                    <div class="form-group" style="margin-bottom: 0">
                        <% for(var z= 0; z < topicType.userTaskEntities.length; z++) { %>
                          ...
                          <p class="form-control-static text-center" title="<%- topicType.userTaskEntities[z].flowName %>" style="word-break: break-all;white-space:
                          normal;">
                            <%- topicType.userTaskEntities[z].flowName %>
                          </p>
                          <% } else { %>
                          <p class="form-control-static text-center" title="<%- topicType.userTaskEntities[z].flowName %>（<%- topicType.userTaskEntities[z].deptAndRoles %>）" style="word-break: break-all;white-space:
                          normal;">
                      ...
                      <div data-flow="<%- topicType.userTaskEntities[w].deptIds %>" data-area="nextFlow">
                        ...
                        <div class="input-group with-clear">
                          <input title="<%- topicType.userTaskEntities[w].nextFlowTaskUserNames %>" type="text" class="form-control" name="taskUsers"
                                 ...
                                 data-value="<%- topicType.userTaskEntities[w].taskUsers %>" placeholder="任务人"
                  ...
                  <td>
                    <div class="form-group" style="margin-bottom: 0">
                      <select class="form-control" name="deptIds" data-fv-notempty="true">
                        <% for(var z= 0; z < topicType.userTaskEntities.length; z++) { %>
                        <% if(topic.declareDeptId === topicType.userTaskEntities[z].deptIds) { %>
                        <option value="<%- topicType.userTaskEntities[z].deptIds %>" selected>
                          <% if(!topicType.userTaskEntities[z].deptAndRoles) { %>
                          <%- topicType.userTaskEntities[z].flowName %>
                          <% } else { %>
                          <%- topicType.userTaskEntities[z].flowName %>（<%- topicType.userTaskEntities[z].deptAndRoles %>）

  ...
  <div class="row">
    <div class="col-xs-12">
      <div class="form-group">
        <label class="col-xs-2 control-label">通知单类型</label>
        <div class="col-xs-10">
          <label class="radio-inline">
            <% if(!info.bookPrintType || info.bookPrintType === '1') { %>
            <input type="radio" name="bookPrintType" value="1" required checked="checked">快印通知单
            <% } else { %>
            <input type="radio" name="bookPrintType" value="1" required>快印通知单
            <% } %>
          </label>
          <label class="radio-inline">
            <% if(info.bookPrintType === '2') { %>
            <input type="radio" name="bookPrintType" value="2" required checked="checked">另件付印单
            <% } else { %>
            <input type="radio" name="bookPrintType" value="2" required>另件付印单
            <% } %>
          </label>
        </div>
      </div>

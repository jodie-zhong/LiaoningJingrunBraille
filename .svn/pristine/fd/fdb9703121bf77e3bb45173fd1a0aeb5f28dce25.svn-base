...
<div class="form-horizontal">
  <div class="row">
    <div class="col-xs-12">
      <div class="form-group ">
        <label class="col-xs-2 control-label">查看结果</label>
        <div class="col-xs-10">
          <label class="radio-inline">
            <% if(!info.handleState || info.handleState === '1') { %>
            <input type="radio" name="handleState" value="1" checked/>通过
            <% } else { %>
            <input type="radio" name="handleState" value="1"/>通过
            <% } %>
          </label>
          <label class="radio-inline">
            <% if(info.handleState === '2') { %>
            <input type="radio" name="handleState" value="2" checked/>不通过
            <% } else { %>
            <input type="radio" name="handleState" value="2"/>不通过
            <% } %>
          </label>
        </div>
      </div>
  ...
  <div class="row">
    <div class="col-xs-12">
      <div class="form-group">
        <label class="col-xs-2 control-label">查看意见</label>
        <div class="col-xs-10">
          <textarea class="form-control" name="handleContentOne" placeholder="查看意见（不超过1024字）" maxlength="1024"><%- info.handleContentOne %></textarea>

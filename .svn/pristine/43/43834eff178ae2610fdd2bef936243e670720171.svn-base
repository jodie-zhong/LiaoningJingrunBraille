...
<script type="text/template" id="workRowTemplate">
	<tr data-type="workload">
		...
		<td>
			<select name="mWorkloadMachineModel" class="form-control" data-type="machine">
				<option value="自动配页(一台机器)">自动配页(一台机器)</option>
				<option value="手动抓页">手动抓页</option>
			</select>
		</td>
		...
		<td class="form-group"><input type="text" class="form-control" name="mWorkloadErrorRate" maxlength="9" /></td>
		<td>
						...
						<table class="table table-striped table-hover">
							...
							<tbody data-area="workload">
								...
								<tr data-type="workload">
									...
									<td>
										<select name="mWorkloadMachineModel" class="form-control" data-type="machine">
											<% if(!info.resultMap.data[i].workLoadBackEntity[j].mWorkloadMachineModel || info.resultMap.data[i].workLoadBackEntity[j].mWorkloadMachineModel === '自动配页(一台机器)') { %>
											<option value="自动配页(一台机器)" selected>自动配页(一台机器)</option>
											<option value="手动抓页">手动抓页</option>
											<% } else if(info.resultMap.data[i].workLoadBackEntity[j].mWorkloadMachineModel === '手动抓页')   { %>
											<option value="自动配页(一台机器)">自动配页(一台机器)</option>
											<option value="手动抓页" selected>手动抓页</option>
											<% } %>
										</select>
									</td>
									...
									<td class="form-group"><input type="text" class="form-control" name="mWorkloadErrorRate" value="<%- info.resultMap.data[i].workLoadBackEntity[j].mWorkloadErrorRate %>" maxlength="9" /></td>
									<td>
										<button class="btn btn-xs btn-success" data-type="work-plus"><i class="fa fa-plus box"></i></button>
									</td>
									<td>
	...
	<div class="row fascicle-top-view" data-time="workTime">
		<div class="col-xs-12">
			<div class="form-group">
				...
				<div class="col-xs-4">
					<p class="form-control-static">
						<%- info.resultMap.data[i].workDate %>
					</p>
				</div>
			</div>
		</div>
					...
					<table class="table table-striped table-hover">
						...
						<tbody>
							...
							<tr data-type="workload">
								...
								<td><%- info.resultMap.data[i].workLoadBackEntity[j].mWorkloadMachineModel %></td>
								<td>
									<%- info.resultMap.data[i].workLoadBackEntity[j].mWorkloadOpenNum %>
								</td>
								<td>
									<%- info.resultMap.data[i].workLoadBackEntity[j].mWorkloadVersionNum %>
								</td>
								<td>
									<%- info.resultMap.data[i].workLoadBackEntity[j].mWorkloadPaperNum %>
								</td>
								<td>
									<%- info.resultMap.data[i].workLoadBackEntity[j].mWorkloadErrorRate %>
								</td>
							</tr>
							<% } %>
						</tbody>

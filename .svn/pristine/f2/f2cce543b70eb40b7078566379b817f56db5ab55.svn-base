...
<div class="form-horizontal">
    ...
    <div class="col-xs-6">
      <div class="form-group">
        ...
        <div class="col-xs-8">
          ...
          <label class="radio-inline"><input type="radio" name="handleState" value="1" checked/>已归还</label>
          <label class="radio-inline"><input type="radio" name="handleState" value="2"/>未归还</label>
          <% } else { %>
          <label class="radio-inline"><input type="radio" name="handleState" value="1"/>已归还</label>
          <label class="radio-inline"><input type="radio" name="handleState" value="2" checked/>未归还</label>
          <% } %>
        </div>
      </div>
    </div>
  ...
  <div class="row" data-flow="<%- info.userTaskEntities[i].flowId %>" data-index="<%- i %>" data-area="nextFlow" style="display: none;">
    ...
    <div class="col-xs-6">
      <!-- 已归还并且是打回过的，不能选人，其他情况（未归还或没打回过）选人 -->
      <% if(i === 0 && info.isReject === '1') { %>
      <div class="form-group">
  ...
  <div class="row">
    <div class="col-xs-6">
      <div class="form-group">
        <label class="col-xs-4 control-label">归还结果</label>
        <div class="col-xs-8">
          <p class="form-control-static"><%- info.handleState === '1' ? '已归还' : (info.handleState === '2' ? '未归还' : '') %></p>
        </div>
      </div>
    </div>
  </div>

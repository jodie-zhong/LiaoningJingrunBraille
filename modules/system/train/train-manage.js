define(function (require, exports, module) {
    ...
    var Module = PageBase.extend(new function () {
         ...
         * 培训查看
         */
        function setTrainCheck(id, isModify) {
        	if (id) {       		
        		var params = {
        			trainId: id
        		};
        		...
        		util.sendRequest({
        			...
        			success: function(resp) {
        				...
        				if (resp.code === 0) {
        					if (isModify) {
        						that.openPage('system/train/train-manage-check', resp.data);
        					}
        				} else{
        					util.showAlert(resp.message || '查询培训信息出错，请稍后重试！');
        				}
        ...
        function initPageEvent() {
        	...
        	that.makeSelectableTable('#trainListTableContainer',
        		function(){
        			...
        			that.$('#btnTrainModify,#btnTrainDel,#btnTrainSubmit,#btnTrainCheck').attr('disabled','disabled');
        			var $row, count = 0;
        			var ids = [], names = [], state = [];
        			...
        			if (count === 1 && currentTrainState === '2') {
        				that.$('#btnTrainCheck').removeAttr('disabled');
        			}
			...
			// 查看
			that.$container.on('click','#btnTrainCheck', function() {
				setTrainCheck(currentTrainId, true);
			});
			// 分页

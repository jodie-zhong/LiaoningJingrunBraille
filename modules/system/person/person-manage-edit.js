define(function(require, exports, module) {
	...
	var Module = DialogBase.extend(new function() {
		...
		function unloadAvatar() {
			var src = that.$('input[name="userHead"]').val();
		...
		function initPageEvent() {
			...
			// 校验登录账号不重复功能
			that.$container.on('blur', 'input[name="userLoginName"]', function () {
				if(!(info && info.userId)){ // 新建的时候
				var $el=$(this);
				if($el.val().length > 2 && $el.val().length < 16){
					util.sendRequest({
						type: 'POST',
						data: {
							userLoginName:$el.val()
						},
						action: 'user/searchSimilarLoginName.action',
						success: function(resp) {
							if(resp.code === 0) {
								// 业务执行成功
								that.$('.repeat-name').show();
							} else {
								// 业务执行失败
								that.$('.repeat-name').hide();
								util.showAlert(($el.val()+resp.message) || '新建登录账号名错误，请稍后重试！');
								$el.val("");
							}
						},
						error: function(xhr) {

...
//		}
		...
		function saveRenewInfo() {
			...
			if(that.validate($form) === true) {
				...
				$form.find('tr[data-area = "contract"]').each(function() {
					$tr = $(this);
					var fileName = $tr.find('input[name="contractFileName"]').val();
					if (fileName) {
		...
		function initPageEvent() {
			...
			// 合同附件删除
			that.$container.on('click', 'button[data-type="contract-minus"]', function() {
				var conmi = $(this).parent().parent();
				conmi.remove();
				contractNum();
				searchButton();
			});
		...
		function initPageData() {
			...
			that.$('input.date-time').datetimepicker({
				...
				locale: 'zh-cn',
				sideBySide: true,
				minDate:info.contractEndDate || new Date()
			});

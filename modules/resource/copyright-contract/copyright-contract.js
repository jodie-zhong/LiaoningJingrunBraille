define(function(require, exports, module) {
	...
	var Module = PageBase.extend(new function() {
		...
		function initPageEvent() {
			...
			// 合同列表选中
			that.makeSelectableTable('#workListTable',
				function() {
					var id = that.$('#contractListTable').find('tr.info').attr('data-contractId');
					searchContractInfo(id);
					searchRenewList(id);
				},
				function() {
					var $row;
					that.$('#contractListTable tr').each(function() {
						$row = $(this);
						if($row.find('i[data-action="check"]').hasClass('fa-dot-circle-o')) {
							currentContractId = $row.attr('data-contractId');
							endData = $row.attr('data-endData');
        					contractNum = $row.attr('data-num');
						}
						if (contractNum === '0') {
        	...
        	that.$container.on('click', '#renewListTable tr', function() {
        		...
        		$(this).addClass('info');
        		currentContractSignId = $(this).attr('data-contractId');

define(function (require, exports, module) {
    ...
    var Module = Base.extend({
         ...
         * 该接口用于查询左侧年月列表树
         */
        yearMonthListAction: 'common/searchNextYearList.action',
         ...
         * 搜索任务流程列表接口
         */
        taskFlowListAction: 'publication/searchTaskList.action',
        ...
        searchTaskFlowList: function () {
            var bookId = this.$('#workListTBody tr').eq(0).attr('data-type');
            var params = {
               	searchBookId: bookId,
               	deploymentKey: 'Publication'
            };
        ...
        searchTaskList: function () {
            ...
            var params = {};
            // 年月
            params.publishYear = this.currentYear;
            // 页码
            ...
            util.sendRequest({
                data: params,
                action: 'publication/searchPublicationList.action',
                success: function (resp) {
                    util.hideLoading();
                    if (resp.code === 0) {
                        // 任务列表
                        this.$('#workListTBody').html(util.template((this.baseTpl || listTpl), {list: resp.data.pageView.records}));
                        // 分页数据
                        this.$('#workListPaging').html(this.buildPagination(resp.data.pageView.pageNow, resp.data.pageView.pageCount));
                        this.searchTaskFlowList();
                        // 回调函数
                        this.onSelectRowChanged(resp.data);
            ...
            util.sendRequest({
                ...
                success: function (resp) {
                    ...
                    if (resp.code === 0) {
                        ...
                        var data = resp.data.split(","), count = 0, node;
                        var newData = [];

...
<tr data-bookId="<%- list[i].bookId %>" data-deploy="<%- list[i].bookDeploymentKey %>" data-taskState="<%- list[i].taskState %>" data-tprTaskType="<%- list[i].tprTaskType %>"
  ...
  <td data-check="true" style="width: 50px;"><i class="fa fa-square-o" data-action="check"></i></td>
  <td title="<%- list[i].bookName %>"><%- list[i].bookName %></td>
  <% if(list[i].bookType === '1') { %>
  <td title="盲文读物">盲文读物</td>	
  <% } else if(list[i].bookType === '2') { %>
  <td title="大字本读物">大字本读物</td>	
  <% } else if(list[i].bookType === '3') { %>
  <td title="普通读物">普通读物</td>	
  <% } else if(list[i].bookType === '4') { %>	
  <td title="有声读物">有声读物</td>
  <% } %>

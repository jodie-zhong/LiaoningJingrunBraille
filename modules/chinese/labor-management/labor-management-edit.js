define(function (require, exports, module) {
    ...
    var Module = DialogBase.extend(new function () {
        ...
        function initPageEvent() {
            ...
            //计算页面数据和保存两位
            that.$container.on('change','input[name="jobInsideReward"],input[name="overtimePay"],input[name="serviceFee"],input[name="benefitAllowance"],input[name="foodAllowance"],input[name="suburbAllowance"]', function () {
                var $el = $(this);
                var price = $el.val();
            ...
            //选择人员
            that.$container.on('click', '.choice-people', function () {
                if(info && info.currentYear && info.currentMonth){
                    that.openPage('chinese/labor-management/choice-user', {currentYear:info.currentYear,currentMonth:info.currentMonth});
                }else{
                    util.showAlert('修改模式下，人员姓名与身份证号不可修改');
                }
            });
        }
        /**
        ...
        this.onBack = function (data) {
            this._super(data);
            if(data.id){
                that.$('input[name="userName"]').val(data.name);
                that.$('input[name="userName"]').attr('data-Id',data.id);
                that.$('input[name="userIdCard"]').val(data.userIdCard);
            }
        };

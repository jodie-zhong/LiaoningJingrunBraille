define(function (require, exports, module) {
    ...
    var Module = DialogBase.extend(new function () {
        ...
        function onContentMinusTwo($dwp, $el) {
            var $par = $dwp.parents('tbody');
            $dwp.parents('tr').remove();
            if(that.$('.lose-count').length>0){
                that.$('.lose-count').each(function () {
                    $(this).change();
                });
            }
            searchTwo($par, $el);
        ...
        function doSave(params) {
            ...
            util.sendRequest({
                ...
                success: function (resp) {
                    util.hideLoading();
                    if (resp.code === 0) {
                        util.showAlert(resp.message || '操作成功！', function () {
        ...
        function initPageEvent() {
            ...
            // 自动计算（第二）
            that.$container.on('change', '.lose-count', function () {
                var $el = $(this);
                // 获取所有name为realPay的值相加
                var tatalN = 0;
                that.$('input[name="realPay"]').each(function () {

define(function (require, exports, module) {
    ...
    var Module = PageBase.extend(new function () {
        ...
        function searchBookList() {
            ...
            util.showLoading()
            util.sendRequest({
                action: 'cnReprint/searchBookToCNReprintList.action',
                data: {},
                success: function (resp) {
                    util.hideLoading();
                    if (resp.code === 0) {
                        //从得到数据提取id
                        var addIds = [], bookNames = [],bookTypeCode = [], bookType = [];
                        ...
                        if (bookDate.idReprint) {
                            var idArr = [];
                            idArr = bookDate.idReprint;
                        ...
                        for (var i = 0; i < addIds.length; i++) {
                            var bookTr = $('<tr data-id="' + addIds[i] + '" data-name="' + bookNames[i] + '" data-bookTypeCode="' + bookTypeCode[i] + '"><td>' + (i + 1) + '</td><td data-check="true"><i class="fa fa-square-o" data-action="check"></i></td><td title="'+ bookNames[i] + '">' + bookNames[i] + '</td><td class="bookType" title="'+ bookType[i] + '">' + bookType[i] +  '</td></tr>');
                            $tbody.append(bookTr);
                        }
                    } else {
                        console.log(resp);

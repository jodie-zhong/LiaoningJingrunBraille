define(function (require, exports, module) {
    ...
    var Module = Base.extend(new function () {
                ...
                success: function (resp) {
                    ...
                    if (resp.code === 0) {
                        ...
                        for(var i=0;i<resp.data.records.length;i++){
                            str='';
                           // 得到字符串
                           var  bookOldPublicationDateStr=resp.data.records[i].bookOldPublicationDate;
                           var  bookOldEditionStr=resp.data.records[i].bookOldEdition;
                           var  bookOldRevisionStr=resp.data.records[i].bookOldRevision;
                           //   将字符串用“，” 分成数组
                            ...
                            for(var k=0;k<newBookOldPublicationDateArr.length;k++){
                               ...
                               if(k!==newBookOldPublicationDateArr.length-1){
                                ...
                                resp.data.records[i].braileStatisticalNewPublicTime=str;
                            }
                        }
                        ...
                        // 将状态赋值给它
                        if(!this.searchParams.searchBookOneKind){
                            if(resp.data.records.length > 0 ){
                                specialState=resp.data.records[0].editIdAndStateEntity.bookEditTprStatisticsState;
                            }else{
                                specialState='';
                            }
                        }
                        ...
                        // 判断是否有滚动条
                        if ( $('#workListTBodyFirst')[0].scrollHeight > $('#workListTBodyFirst')[0].offsetHeight) {
                            // 有滚动条
                            $('#workListTBodyFirst').css({'width':'7416px'});
                        } else {
                            // 没有滚动条
                            $('#workListTBodyFirst').css({'width':'7410px'});
                        }
                        // 回调函数
        ...
        this.setState = function () {
            ...
            if (that.$('#workListTBody tr').attr('data-noselect') && (specialState !=='2' && specialState !=='3')) {
                // 没有数据则添加亮
        ...
        function initPageEvent() {
            ...
            that.$container.on('click', 'span[data-href]', function () {
                var $el = $(this);

define(function (require, exports, module) {
    ...
    var Module = tree.extend(new function () {
        this.yearMonthListAction = 'common/searchYearNextMonthList.action';
        var that;
        var currentMonth='';
        var currentYear='';
        ...
        function searchReportList(){
            ...
            // 重置下边选题列表
            // 将底部修改与新增按钮置灰
            that.$('#btnConferenceReportAdd,#btnConferenceReportDel,#btnConReportChecking').attr('disabled', 'disabled');
        ...
        function conferenceList(){
            ...
            // 将底部修改与新增按钮置灰
            that.$('#btnConferenceReportAdd,#btnConReportChecking,#btnConferenceReportDel').attr('disabled', 'disabled');
                    ...
                    success: function (resp) {
                        ...
                        if (resp.code === 0) {
                            ...
                            if(currentReportState==='1'){
                                that.$('#btnConferenceReportAdd,#btnConReportChecking').removeAttr('disabled');
                            }else {
                                that.$('#btnConferenceReportAdd,#btnConReportChecking').attr('disabled','disabled');
        ...
        function conferenceSubmit() {
            ...
            if(!currentSignFlag){// 针对选题大会拒绝和待定，直接提交
                var obj={};
                util.showConfirm('确定要提交么？',function () {
                    ...
                    params1.topicId = '';
                    params1.taskId = '';
                    params1.topicMeetingId = params.topicMeetingId;
                    params1.meetingEntities = [];
                    arr.push(params1);
                    obj.submitMeetingEntities=arr;
                    util.sendRequest({
                        data: obj,
        ...
        function checkFn() {
            ...
            currentSignFlag=that.$('#conferenceListTable').find('tr[checked="checked"]').attr('data-signFlag');
            that.$('#btnConferenceModify, #btnConferenceDel, #btnConferenceReg,#btnConferenceSubmit,#btnConferenceSearch').attr('disabled','disabled');
            ...
            util.sendRequest({
                ...
                success: function(resp) {
                    ...
                    if (resp.code === 0) {
                        ...
                        allReportInfo.topicReportId=currentConferenceIdBottom;
        ...
        function initPageEvent() {
            ...
            that.$container.on('click', '#btnConferenceAdd', function () {
                that.openPage('topic/topic-conference/topic-conference-edit', {month:currentMonth,year:currentYear});
            });
            //修改
            ...
            //  查重（底下查重按钮）
            that.$container.on('click','#btnConReportChecking', function () {
                that.openPage('topic/topic-conference/topic-rechecking',{currentConferenceId:currentConferenceIdBottom});
            });
        ...
        function selectTree(a,b) {
            ...
            if((currentYear===year && (currentMonth===month || currentMonth===(month+1))) || ((currentYear=== (year +1) && currentMonth===1))){// 处理12月，下一个月是下一年1月的问题

            ...
            this.$("tbody[data-name='trueOrFalseTb']").children().each(function () {
                ...
                for (var i = 0; i < params.rightWrongList.length; i++) {
                    if (params.rightWrongList[i].rigthWrongId === rightWrongId) {
                        params.rightWrongList[i].checkResult = $(this).find("input[name='checkResult']").prop('checked') ? "1" : "2";
                        params.rightWrongList[i].checkIdea = $(this).find("input[name='checkIdea']").val();
                    }
                }
            });
            // 差错表数据

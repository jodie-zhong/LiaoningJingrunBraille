...
<div class="form-horizontal">
  <div class="dialog-options">
    <button class="btn btn-xs btn-primary" id="btnExport" data-type = "<%- info.zPublishPlanReportId %>">导出</button>
      ...
      <div class="form-group ">
        ...
        <div class="col-xs-10">
          <label class="radio-inline">
            ...
            <input type="radio" name="handleState" value="1" checked/>同意上报
            <% } else { %>
            <input type="radio" name="handleState" value="1"/>同意上报
            <% } %>
          </label>
          <label class="radio-inline">
            <% if(info.handleState === '2') { %>
            <input type="radio" name="handleState" value="2" checked/>不同意上报
            <% } else { %>
            <input type="radio" name="handleState" value="2"/>不同意上报
            <% } %>
          </label>
        </div>
      </div>
  ...
  <div class="row">
    <div class="col-xs-12">
      <div class="form-group">
        ...
        <div class="col-xs-10">
          <textarea class="form-control" name="handleContentOne" placeholder="审核意见（不超过1024字）" maxlength="1024"><%- info.handleContentOne %></textarea>
  ...
  <% if(info.userTaskEntities[i].flowId === 're_publicationAudit') { %>
  <div class="row" data-flow="<%- info.userTaskEntities[i].flowId %>" data-area="nextFlow" style="display: none;">
    ...
    <div class="col-xs-6">
      ...
      <div class="form-group">
        ...
        <div class="col-xs-8">
          <input type="text" class="form-control" name="taskUsers" title="<%- info.userTaskEntities[i].tprFlowName %>"
          value="<%- info.userTaskEntities[i].tprFlowName %>" data-value="<%- info.userTaskEntities[i].taskUsers %>" readonly>
        </div>
      </div>
      <% } %>
    </div>

...
<div class="form-horizontal">
  ...
  <div class="row" data-type = "agree">
    <div class="col-xs-12">
      <div class="form-group ">
        <label class="col-xs-2 control-label">成书时间</label>
        <div class="col-xs-3">
          <label class="radio-inline">
            <% if(!info.publishBookTime || info.publishBookTime === '1') { %>
            <input type="radio" name="publishBookTime" value="1" checked/>当天
            <% } else { %>
            <input type="radio" name="publishBookTime" value="1"/>当天
            <% } %>
          </label>
          <label class="radio-inline">
            <% if(info.publishBookTime === '2') { %>
            <input type="radio" name="publishBookTime" value="2" checked/>隔天
            <% } else { %>
            <input type="radio" name="publishBookTime" value="2"/>隔天
            <% } %>
          </label>
          <label class="radio-inline">
            <% if(info.publishBookTime === '3') { %>
            <input type="radio" name="publishBookTime" value="3" checked/>其他
            <% } else { %>
            <input type="radio" name="publishBookTime" value="3"/>其他
            <% } %>
          </label>
        </div>
        <div class="col-xs-5" data-type = "bookTime">
        	<input type="text" class="form-control" value="<%- info.publishBookTimeRemark %>" name="publishBookTimeRemark"  />
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="row">
    <div class="col-xs-12">
      <div class="form-group ">
        ...
        <div class="col-xs-5">
          <p class="form-control-static">
            <% if(info.publishBookTime === '1') { %>
            当天
            <% } else if(info.publishBookTime === '2') { %>
            隔天
            <% } else if(info.publishBookTime === '3') { %>
            其他
            <% } %>
          </p>
        </div>
        <% if (info.publishBookTime === '3') { %>
        <div class="col-xs-5">
        	<p class="form-control-static"><%- info.publishBookTimeRemark %></p>
        </div>
        <% } %>
      </div>
    </div>

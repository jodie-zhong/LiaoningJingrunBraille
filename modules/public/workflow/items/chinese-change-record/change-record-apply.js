...
//		name : '',
		...
		buildSelect: function() {
			...
			$form.find('div[data-area="changeRecordItem"]').each(function() {
				$changeName = $(this);
				selectList.push($changeName.find('input[data-name="name"]').val())
			});
			this.$('#changeContentSelect option').remove();
			var $sel = this.$('#changeContentSelect'),
				$option;
				
			for(var i in changeRecordConfig) {
				if(changeRecordConfig.hasOwnProperty(i) && selectList.indexOf(i) < 0) {
		...
		getFormData: function() {
			...
			$form.find('div[data-area="changeRecordItem"]').each(function() {
				...
				params.changeFileList.push({
					...
					changeFileAfter: $list.find('input[data-name="after"]').val() || $list.find('textarea[data-name="after"]').val(),

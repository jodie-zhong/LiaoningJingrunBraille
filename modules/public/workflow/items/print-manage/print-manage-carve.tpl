		...
		<div class="row fascicle-top" data-time="workTime">
			<div class="col-xs-12">
				<div class="form-group">
					<label class="col-xs-2 control-label">工作日期<span class="must">*</span></label>
					<div class="col-xs-4">
						<input type="text" class="form-control date-time" value="<%- info.resultMap.data[i].workDate %>" name="mWorkloadDatetime" placeholder="工作日期" maxlength="20" data-fv-err="tooltip" data-fv-notempty="true" data-fv-notempty-message="请选择工作日期" />
					</div>
					<div class="col-xs-6">
						<button class="btn btn-xs btn-success" data-type="day-plus"><i class="fa fa-plus box"></i></button>
						<button class="btn btn-xs btn-danger" data-type="day-minus"><i class="fa fa-close box"></i></button>
					...
					<table class="table table-striped table-hover">
						...
						<tbody>
							...
							<tr data-type="workload">
								...
								<% if(info.resultMap.data[i].workLoadBackEntity[j].mWorkloadMachineModel === '1') { %>
								<td>一台机器(美国)</td>
								<% } else if(info.resultMap.data[i].workLoadBackEntity[j].mWorkloadMachineModel === '2') { %>
								<td>一台机器(比利时)</td>
								<% } else { %>
								<td>一台机器</td>
								<% } %>

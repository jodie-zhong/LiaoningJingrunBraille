            ...
            this.$("tbody[data-name='trueOrFalseTb']").children().each(function () {
                ...
                for (var i = 0; i < params.qualityRightWrongEntities.length; i++) {
                    if (params.qualityRightWrongEntities[i].rigthWrongId === rightWrongId) {
                        params.qualityRightWrongEntities[i].isConfirm = $(this).find("input[name='isConfirm']").prop('checked') ? "1" : "2";
                        params.qualityRightWrongEntities[i].confirmCause = $(this).find("input[name='confirmCause']").val();
                    }
                }
            });
            // 差错表数据

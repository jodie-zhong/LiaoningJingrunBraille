    ...
    module.exports = ItemBase.extend({
        ...
        recalculateDiff: function () {
            ...
            this.$("tbody[data-name='trueOrFalseTb']").children().each(function () {
                var checked = that.data.isEdit ? $(this).find("input[name='checkResult']").prop('checked') : ($(this).attr('data-check-result') === '1');
            ...
            this.$("tbody[data-name='trueOrFalseTb']").children().each(function () {
                ...
                for (var i = 0; i < params.qualityRightWrongEntities.length; i++) {
                    if (params.qualityRightWrongEntities[i].rigthWrongId === rightWrongId) {
                        params.qualityRightWrongEntities[i].checkResult = $(this).find("input[name='checkResult']").prop('checked') ? "1" : "2";
                        params.qualityRightWrongEntities[i].checkIdea = $(this).find("input[name='checkIdea']").val();
                    }
                }
            });
            // 差错表数据
        ...
        initPageEvent: function () {
            ...
            this.$container.on('change', 'input[name="checkResult"]', function () {
                ...
                if ($(this).prop('checked')) {
                    ...
                    if (allChecked) {
                        $(this).parents('table').find('input[name="checkResultBatch"]').prop('checked', 'checked');
                        // 没有输入框不显示必填星号
                        that.$(".uncheckMust").hide();
                    }
                    // 隐藏输入框
                    selectionRow.find("input[name='checkIdea']").parent().hide();
                }
                // 不同意
                else {
                    // 去掉全选
                    $(this).parents('table').find('input[name="checkResultBatch"]').removeProp('checked');
                    // 有输入框显示必填星号
                    that.$(".uncheckMust").show();
                    // 显示输入框
                    selectionRow.find("input[name='checkIdea']").parent().show();
                }
                // 计算差错率
                that.recalculateDiff();
            });
            ...
            this.$container.on('change', 'input[name="checkResultBatch"]', function () {
                ...
                if ($(this).prop('checked')) {
                    ...
                    that.$(".uncheckMust").hide();
                    // 隐藏输入框
                    table.find("input[name='checkIdea']").parent().hide();
                }
                // 不同意
                else {
                    // 设置复选框不选中
                    $(this).parents('table').find('input[name="checkResult"]').removeProp('checked');
                    // 有输入框显示必填星号
                    that.$(".uncheckMust").show();
                    // 显示输入框
                    table.find("input[name='checkIdea']").parent().show();
                }
                // 计算差错率
                that.recalculateDiff();
            });
        ...
        initPageData: function () {
            ...
            this.$('input[name="checkResult"]').each(function () {
                ...
                if ($(this).prop('checked')) {
                    ...
                    that.$(".uncheckMust").hide();
                    // 隐藏输入框
                    $(this).parents('tr').find("input[name='checkIdea']").parent().hide();
                }
                // 不同意
                else {
                    // 去掉全选
                    ...
                    that.$(".uncheckMust").show();
                    // 显示输入框
                    $(this).parents('tr').find("input[name='checkIdea']").parent().show();
                }
            });
            if (allChecked) {
